    ...
    if (empty($_POST['employee_id'])) {
        ...
        if (mysqli_num_rows($position_check_result) > 0) {
            ...
            if (mysqli_query($con, $sql)) {
                // If room_id is provided, update the room table with the employee_id
                if ($room_id) {
                    // Check if the room_id exists
                    $room_check_query = "SELECT * FROM room WHERE RoomID = '$room_id'";
                    $room_check_result = mysqli_query($con, $room_check_query);

                    if (mysqli_num_rows($room_check_result) > 0) {
                        $update_room_query = "UPDATE room SET EmployeeID = '$employee_id' WHERE RoomID = '$room_id'";
                        if (mysqli_query($con, $update_room_query)) {
                            $_SESSION['success_message'] = "Employee and room assignment updated successfully";
                        } else {
                            $error_message = "Error updating room assignment: " . mysqli_error($con);
                        }
                    } else {
                        $error_message = "Invalid Room ID. Please make sure the Room ID exists in the room table.";
                    }
                } else {
